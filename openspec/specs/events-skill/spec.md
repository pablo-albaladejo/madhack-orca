...
- **THEN** it calls Ticketmaster API with the city and upcoming Friday-Sunday date range and returns a DataPart with events and a TextPart summary

#### Scenario: Events skill without API key
- **WHEN** the events skill is called but `TICKETMASTER_API_KEY` is not set
- **THEN** it returns realistic mock event data for the requested city (5 cities: Madrid, Barcelona, Valencia, Lisbon, Sevilla)
